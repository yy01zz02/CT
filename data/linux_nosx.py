...
for model_path in model_list:
    ...
    for name in data_names:
        ...
        # 读取已经处理过的数据
        if os.path.exists(f'exp/{model_name}/{name}/prompt_not_cot_not_sx.json'):
            with open(f'exp/{model_name}/{name}/prompt_not_cot_not_sx.json', 'r', encoding='utf-8') as ff:
                try:
        ...
        for item in data:
            ...
            prompt = prompt_not_content(bug, issue)
            ...
            # 保存结果路径
            json_path = f'{folder_path}prompt_not_cot_not_sx.json'
